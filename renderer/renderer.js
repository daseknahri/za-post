        ...
        <div class="account-posting-method" style="margin: 12px 0; padding: 10px; background: #1e293b; border-radius: 8px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <span style="font-size: 12px; color: #94a3b8;">🔄 Posting Method:
              <span class="help"><span class="help-q" tabindex="0" role="button" aria-label="Help">?</span><span class="help-tip"><b>Posting Method — how posts are assigned to this account each cycle.</b><br><b>🎯 Post to All Groups:</b> posts EVERY eligible post (capped by Max posts per cycle) to all its groups. All accounts post the same set — use to blanket many groups from each account.<br><b>🔀 Random (Shuffle):</b> same, but post order is shuffled (same shuffle for all accounts that cycle).<br><b>🎯🔒 One Post Per Account (Unique):</b> exactly ONE post per cycle, dealt round-robin so each post is published once across all accounts. Spreads a library across accounts with no repeats. If undealt posts are fewer than active accounts, the surplus accounts wait that cycle (the plan shows "waits — pool exhausted") — enable Loop Campaign to recycle, or click Reset Campaign Rotation.<br><b>🔀🔒 Random (No Repeat):</b> like Unique, but the deal order is shuffled.<br><b>📋 Sequence (Progressive):</b> one post per cycle in declared order, rotating which account gets which across loops.<br><b>📅 Daily Rotation:</b> 1 new post per LOCAL day, per agent. Each agent independently tracks which post it posted yesterday and advances its own pointer forward — no shared dealt-set. postsPerGroup, Loop Campaign, and Completion Mode are ignored; it self-paces to 1/day in either schedule mode.<br><b>🗓️ Campaign Plan:</b> agents that share the same group-set split the whole post library between them, 1/agent/day, so every group receives every post once across the team. Loop Campaign ON = reshuffle a fresh round when the batch finishes; OFF = stop after it. No Daily schedule needed — it self-paces to 1/day.<br><i>Distinct content per account only happens when posts ≥ accounts (the 🔒 modes).</i></span></span>
            </span>
          </div>
          <select id="posting-order-${account.name}" onchange="updatePostingOrder('${account.name}', this.value)" style="width: 100%; padding: 8px 12px; background: #1f2937; border: 1px solid #374151; border-radius: 6px; color: #e5e7eb; font-size: 13px; cursor: pointer;">
            ...
            <option value="sequence" ${account.postingOrder === 'sequence' ? 'selected' : ''}>📋 Sequence (Progressive)</option>
...
const SETTINGS_HELP = [
  ...
  ['Max posts per cycle', '<b>Max posts per cycle.</b> How many DISTINCT posts an account publishes each cycle — and each of those posts goes to ALL of the account\'s groups. 0 = no cap (all eligible). <b>Applies only to "Post to All Groups" and "Random".</b> Unique, Random No-Repeat, Sequential, Daily Rotation and Campaign Plan always post exactly 1 per cycle and ignore this.'],
  ...
  ['Max Cycles', '<b>Max Cycles.</b> Stop automatically after this many full posting cycles. 0 = run indefinitely. Daily Rotation runs ~1 cycle/day. Ignored while Completion Mode (or a finishing Campaign Plan) is still delivering — so it can\'t cut a finite campaign short; leave it at 0 there.'],
  ...
  ['Loop Campaign', '<b>Loop Campaign (Unique / Sequential / Campaign Plan).</b> OFF = the campaign ends after the library is delivered once, then stops. ON = it recycles forever — re-distributing the library and rotating which account posts which content (Campaign Plan reshuffles a fresh round). No effect on Post-to-all / Random / Daily Rotation.'],
