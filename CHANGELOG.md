...
## [1.0.138] — 2026-07-18 — Speed audit + daily-roster auto-re-arm: swap accounts → press Start (no more Start Fresh)

Suite **481/481**, antispam 34/34. From a deep, adversarially-verified audit of the live logs. **Headline: the campaign slowness is CONTENTION, not code** — 8–10 Chrome renderers on a 4-core laptop sharing one IP. The per-post nav→confirm time measured **19s at 1–2 concurrent vs 90–164s at 6–11** (the CPU-light steps stay flat; the DOM-readiness loops explode on a starved renderer). **The #1 speed fix is a config the operator owns: lower `realIpMaxConcurrent` 8 → 4–5** — throughput is unchanged (daily caps bind total volume), partials/reserve-churn largely vanish, and shared-IP ban pressure drops. The code fixes below are the safe wins around it.

- **[HIGH][daily-workflow] Daily auto-re-arm for full-batch-daily.** The operator swaps accounts every day; under `loopCampaign:false` the completed one-shot plan re-loaded the next day and delivered **~0** (a swapped-in account was benched with no round boundary to un-bench it), so the only fix was a manual Start Fresh every day. New `_dailyRearmIfNeeded`: on a new local day with yesterday's batch fully delivered, it rebuilds the plan for **today's roster** automatically — picking up swapped-in accounts, dropping removed ones — **but only if the post library changed.** Re-dealing the *same* posts to the same groups is a cross-day spam burst (campaign-plan has no durable per-(post,group) guard), so an unchanged library stays finished and the operator is told to add posts. Mirrors the loopCampaign reset's atomicity exactly (null pointers + rebuild + one save) so it can never create the plan-null-with-live-pointers state that shipped the v1.0.96–1.0.113 double-post. New roster-independent `_libraryHash` powers the gate. +7 tests (same-library refuses, new-content rebuilds, swapped-in gets a slice, atomicity, same-day/loop/empty-roster guards).
- **[MEDIUM][speed] Composer-open attempts trimmed 4 → 2** (`worker.js` `composerOpenAttempts`). Measured over 3 live days: attempt 2 recovered **167** composers; attempts 3 & 4 recovered **3 & 0**, while each terminal attempt burns a ~9s hard `waitForSelector` on a starved feed — so 4 attempts wasted **~18s of dead-wait per fully-starved feed** for near-zero recovery. Read-only pre-publish path (nothing clicked/submitted) → can never double-post or touch the publish-confirm. Attempt 2 kept (it does the bulk of recoveries).
- **[LOW][diagnostics] Two misleading logs fixed.** (a) The two-phase publish log claimed *"commenting via the direct link (feed re-scan skipped)"* when Phase 2 actually uses the id-strict group feed-scan (the permalink is deliberately retired for identical-caption wrong-post safety) — this exact contradiction seeded a wrong "permalink is wasted" hypothesis. Now truthful, gated on `_twoPhase`. (b) A failed image-vary was hard-coded to blame *"webp"*, but every library image is JPEG — the real cause was `writeAsync` failing on a near-full disk under load. `imageVary` now surfaces the real error (`lastVaryError`) and only says "webp" after a magic-byte check (`isWebp`); otherwise it points at disk/load.
- **REJECTED (verified unsafe): re-enabling the Phase-2 comment permalink.** The permalink IS captured 100% and carried into Phase 2, but passed `null` on purpose (v1.0.83) — for this fleet (identical captions, one account) the direct-permalink path was wrong-post-prone, and the prefetch pre-loads the group FEED, so re-enabling it would comment against a multi-article feed page and reproduce the v1.0.44 "all comments on one post" incident. Left alone.
- **The crash-resume double-post needs no new code.** The durable guard already exists (`appendInflight` fsyncs per-post at `markDelivered`; the fold reconstructs the pointer on every Start). The residual window — FB publish succeeds → process killed before the fsync — is irreducible, and ADR-0021 already chose a ~1% strand over the ban risk of an eager pre-publish write. The real lever is proxies. **Operational rule stands: stop via the app's ■ Stop, never a force-kill; the app must be launched detached, never as a session-bound task.**

## [1.0.137] — 2026-07-18 — Parallel-accounts control + per-IP cap raised to 20; a midnight-flaky test fixed

Suite **472/472**, antispam 34/34.

- **"Parallel accounts" is now a Settings control (1–20).** It had no UI input — Quick Setup auto-computed it from the active-account count, so an operator couldn't set it (e.g. to 10). Added the input, wired load/save, and **stopped Quick Setup from lowering a manually-set value** (it now prefers proxies → the operator's setting → active count, instead of always overwriting with the active count).
- **"Max browsers on one IP" (`realIpMaxConcurrent`) ceiling raised 8 → 20**, in the store clamp, the engine gate (`_realIpMax`), and the UI. On a single shared IP the effective concurrency is `min(parallelAccounts, realIpMaxConcurrent)`, so the old cap of 8 silently prevented >8 browsers no matter what parallelAccounts was. Now an operator who explicitly wants more can — the hardware (RAM/CPU) ceiling still bounds it to what the machine holds, and the ban exposure on that IP is the operator's to own (throttling was observed at 8).
- **[test] A midnight-boundary flake fixed.** `[postsPerCycle] postsToday counts every post delivered` keyed its per-day delivery map by `toISOString()` (UTC day) while the engine's `postsTodayDate` uses `_localDayKey()` (local day). Run just after the local date rolls, the two day-keys diverge, no account matches, and the run's `checked > 0` guard fails spuriously — nothing to do with the accounting it verifies. The test now computes the local day-key the engine's way; passes repeatedly.
- **Operational note (not a code bug):** a live campaign-plan run re-delivered ~12 (post,group) pairs and over-delivered past per-account slices after the app was **force-killed mid-cycle and its run-active flag cleared** — the crash-fold then did not reconstruct the un-persisted in-flight deliveries. Campaign-plan has no durable per-(post,group) guard (the partition is the guard), so an unclean kill is a re-post risk. Mitigation is procedural: stop via the app's ■ Stop (persists pointers), never a force-kill; Start Fresh after an unclean exit clears stale pointers. A durable per-(post,group) ledger for campaign-plan (ADR-0023 Phase 5) would close it structurally.

## [1.0.136] — 2026-07-17 — Full-batch-daily: post the whole batch in one run, no dials — a UI preset, zero engine changes

Suite **472/472**, antispam 34/34. Default OFF → byte-identical to the prior engine.

- **`fullBatchDaily` — the simple mode the operator asked for.** One checkbox: each campaign account posts its whole share of its batch in one daily run, then stops ("deliver all, then done"). Volume is controlled by *how many posts you put in the batch* — 30 posts across 3 accounts = 10 each; 3 across 3 = 1 each — with **no numbers to set**: the `cyclesPerDay` and `postsPerCycle` dials are computed from the batch and hidden.
- **It is a UI PRESET, not an engine mode — the decisive safety choice.** When the box is ticked, the save expands it into settings the engine *already runs safely*: `cyclesPerDay = postsPerCycle = maxSlice` (the largest per-account batch share, clamp 1..50), `loopCampaign:false`, `completionMode:true`, `scheduleMode:daily`. The orchestrator has **zero changes** and never reads the flag, so a double-post/re-burst is structurally impossible and OFF is byte-identical for free. A design pass explicitly rejected the alternative (a mode flag that rewires the quota gates) after verification found it introduces two new ban-axis bugs — an empty-slice `N=0` campaign wedge and a crash-fold under-count re-burst; the preset avoids both because `maxSlice ≥ 1` and the quota source is unchanged.
- **`store.campaignMaxSlice(accounts, posts)`** (new, exported, tested) derives the volume: clusters enabled campaign accounts by shared groups + post-set (mirroring `_computeCampaignPlan`), computes each batch's `ceil(pool / accounts)`, returns the largest, clamped 1..50, and flags `capped` when a batch needs >50/account (the UI warns rather than silently truncating). The renderer mirrors it (no `require` in the renderer context).
- **Verified end-to-end** on the harness: a 3-account/6-post batch under the preset delivers the full batch in **one cycle, each account its whole slice once, 0 duplicates, 0 reloops** — "deliver all, then done." Derivation pinned by 2 unit tests (30/3=10, 3/3=1, 0→1 floor, 120/2→50 capped, largest-batch-wins, disabled/mod/unique excluded).
- **Full-batch-daily is now the DEFAULT in Quick Setup for campaign-plan** — so the operator gets it without touching Settings ("we don't need to set it in the admin"). This also **fixes a real defect**: QS wrote neither `cyclesPerDay` nor `postsPerCycle`, so a wizard campaign silently delivered **1 post/account/day regardless of batch size**. QS now derives `maxSlice` from the configured roster and writes the same safe preset (`loopCampaign:false`, `completionMode:true`, `cyclesPerDay=postsPerCycle=maxSlice`), applied after the setting spreads so it wins. Only campaign-plan; daily-rotation/sequence keep their own pacing. Verified: what QS writes equals what the engine reads.
- **Daily recurrence is by design NOT in-engine** (that would re-deal the same library daily = the ban axis). The batch delivers once; the next day's run posts whatever posts are in the batch then — so you feed new posts, matching "post as many as we give him."

## [1.0.135] — 2026-07-17 — The comment-failure breaker was inert: a comment-dead account posted commentless forever

Suite **470/470**, antispam 34/34. Fixes the operator-reported "even for comment limit the machine keeps posting without comment."

- **[HIGH][ban-exposure] The persisted comment-health breaker never actually rested an account.** When an account's comments stop landing, the breaker (`orchestrator.js:1587`) sets a cool-down (`rateLimitedUntil`, `rlStrikes++`). But a comment-dead account still *posts fine* — the link just never lands — so it satisfies `posted>0 && !flag`, and the "recovered → clear cool-down" branch **in the same `store.update` callback** (`:1650`) wiped the rest the breaker had just set. Net: the breaker was a no-op, and the account kept publishing commentless indefinitely — full daily-cap burn + full shared-IP ban exposure for zero value (the link is the payload). Reproduced at runtime: 8 cycles of lost comments → **0 surviving rests, rlStrikes stuck at 0**. The comment at `:1576-1580` describes the symptom; the wipe is why the documented fix didn't hold. Live 14-day log: **186 definitely-commentless deliveries (a floor — the ~70% "unconfirmed" from max-mode's skipped re-scan are invisible), 122 of them two config-broken accounts that emitted 92 and 30 commentless in a row** instead of resting after ~3.
- **The fix is a one-cycle sentinel, not a behavior change:** a local `_commentRested` flag set when the breaker fires excludes that cycle from the recovered-clear. A genuine *post* rate-limit recovery still clears (its cycle never trips the comment breaker); a comment rest that genuinely recovers is cleared on a **later** clean cycle (comments landing again). It only ever *tightens* resting — it never re-posts anything and never touches the dealt-set. `unconfirmed` still maps to unknown (neither win nor loss), so a healthy max-mode account cannot be falsely rested.
- **[MEDIUM][ban-exposure] Group-aware comment threshold — a 2-group account no longer burns two full cycles before resting.** The breaker's flat threshold of 3 meant a 2-group account (the operator's entire fleet — 25 of 26 accounts) could only reach it across **two** cycles = **four** commentless posts, because one cycle produces at most 2 losses. The threshold is now `max(2, min(3, groupCount))`: a 2-group account rests after **one** fully-failed cycle (2 commentless, halved), a wide account keeps the original confidence of 3, and the floor of 2 means a single isolated drop never rests any account. Only DEFINITE losses count (`unconfirmed` stays unknown), so a healthy max-mode account is untouched.
- **The existing `[live]` tests re-implemented the accumulator inline, so they passed while the real code wiped the rest** — the exact false-confidence the N-cycle discipline exists to catch. Added 5 tests that drive the **real** `_recordAccountOutcome` across cycles through a temp store: (1) a comment-dead account actually rests — mutation-verified (reverting the guard fails only this test); (2) a 2-group account rests after one fully-failed cycle — mutation-verified against the flat threshold; (3) a wide 3-group account keeps threshold 3; (4) an `unconfirmed`/unknown outcome never falsely rests a healthy account; (5) comments recovering clears the rest so a recovered account is not stuck.
- **Also mapped (not yet built), from the same investigation:** P1 "move fast on a limit" is **already fast** — the pipelined pool never stalls on a limited account and a cooling account is skipped before any browser launches (measured: after all 10 live rate-limits the fleet kept posting within seconds). P2 "too much waiting when healthy" is **structural barrier idle** (~85% of each cycle) governed by the `postsPerCycle` setting (unset=1) — raising it 3–5 collapses the idle, at a spread-vs-burst ban trade the operator owns. Both documented for a follow-up.

## [1.0.134] — 2026-07-17 — postsPerCycle: collapse the per-cycle barrier so a batch drains in one continuous run

Suite **460/460**, antispam 34/34. Default `postsPerCycle: 1` → byte-identical to the prior engine; the whole change is inert until an operator raises it.

**The problem the operator found: a cycle is a BARRIER.** Every account must finish before the next cycle starts, so an account that ends early idles until the slowest one lands — and over N cycles you pay the *slowest* account's time N times (`N × E[max]`) instead of once (`max` of the per-agent sums). Measured on the live logs: agents idled **~75% of each cycle** (much of it queueing for one of only 3 browser slots, but the barrier is real and grows with N). The batch model the operator wants — "5 accounts, 50 posts, one continuous run" — was blocked by campaign-plan delivering exactly **one post per cycle**, hardcoded.

- **`postsPerCycle` (new, default 1 = off).** How many of its slice a campaign agent walks back-to-back in one cycle. Set it equal to the posts-per-day budget to get one continuous run per account per day, no barrier. **It never raises the daily total** — the picker asks for `min(postsPerCycle, cyclesPerDay − postsToday)`, so it changes *when* the day's posts land, never *how many*. Proven end-to-end on the operator's real shape (5 accounts, one shared group-set, 50 posts): `postsPerCycle:10` → all 50 land in cycle 1, 10 per account, **0 duplicates**, and `postsToday` reads 10 so the daily quota still binds.
- **The safe invariant, and why it needed no owed data-model change.** A prior audit judged multi-post cycles to require widening the single-slot `_owed` ledger (the machinery ADR-0021 was rejected for touching), because N posts could each partially deliver and only one obligation fits. The fix instead **stops the posts loop at the first post that does not reach every group** — so a cycle has at most ONE partial and it is always LAST, which is exactly what one owed slot holds. Conservative by construction (only ever ends a cycle early; at `postsPerCycle:1` it is unreachable → byte-identical). The other guards (`_cycleDelivered`, the pointer, the reloop) make its absence a temporary within-round under-delivery rather than a double-post — so the audit's BLOCKER severity was overstated — but keeping it makes the single-slot ledger *provably* sufficient rather than empirically so.
- **Five sites each independently hardcoded "one post per agent per cycle"; all five fixed and mutation-verified.** (1) the picker returns up to `want` posts; (2) the pointer records `dealtIds[LAST]`, not `dealtIds[0]` — naming the first post of a batch advances the pointer by one while N were delivered, re-posting the rest next cycle (the measured 465-posts/agent/day, 93.5%-re-post over-post); (3) `postsToday` counts each distinct (agent, POST) once, not once per cycle — a per-cycle count is what let the day run to 17.3× the quota; (4) the owed recording keys on the last-dealt post; (5) the crash-fold credits `+posts.size` and its highest-`q` tie-break uses `>=` so it names the LAST surviving post (every post in a cycle shares one `_runSeq`, allocated before the loop — a strict `>` would rewind to the first and re-post the rest on resume). Each of the four hazardous sites has a test that fails when the fix is reverted; the harness's other guards initially masked three of them, so the fixtures were sized so the slice outlasts one cycle, which is the only place the cross-cycle machinery differs.
- **`parallelAccounts` default context + the live set to 5.** On a real IP effective concurrency is `min(parallelAccounts, realIpMaxConcurrent)`; the live fleet had `parallelAccounts:3` binding under an inert `realIpMaxConcurrent:5`, so raising the former to 5 lifts one-browser-per-account to five (**+67%**, the real throughput lever per ADR-0023). Set in the live config only; the store default stays 2 for new installs.
- **UI:** a new **Posts per cycle** field, and **Cycles per run** relabelled **Posts per account per day** with copy that ties the two together ("set them equal for one continuous run").

## [1.0.133] — 2026-07-17 — Posts-per-day: the dial the operator actually wanted, and the rotation save stops eating fields

Suite **453/453**, antispam 34/34.

**The operator asked for "1 cycle/day, up to 30 posts per account". They already had the model — they were blocked by a clamp.** Campaign-plan and daily-rotation deliver exactly ONE post per account per cycle (`orchestrator.js` picker, `return post ? [post] : []`) and `_dailyQuotaBlocks` stops the account at `N = cyclesPerDay` — so **`cyclesPerDay` IS "posts per account per day"**, by construction. In `daily` mode those N cycles fire one after another from `dailyPostTime` and then rest until tomorrow, which is precisely "one run per day with N posts in it". `cyclesPerDay` was clamped to **1..20** in seven places, so 30 posts/day was literally inexpressible and the only apparent alternative was "make an account post more per cycle" — which is a re-burst (below).

- **`cyclesPerDay` ceiling 20 → 50, and the clamp is now ONE authority (`store.dailyPostQuota`, `MAX_POSTS_PER_DAY`).** It was a bare `Math.min(20, …)` copy-pasted at seven sites (store, the quota gate, the crash-fold, the daily-schedule gate, the round reset, and two label helpers) — six of them in the engine, free to drift apart. Verified: the gate now allows posts 20–29 at `cyclesPerDay:30` and blocks at 30; `cyclesPerDay:1` is byte-identical; 999 rails to 50; garbage → 1. The UI dial is relabelled from *"Cycles per run"* to **"Posts per account per day"**, which is what it has always done.
- **REJECTED (measured, not assumed): making the picker return N posts per cycle.** It buys **≈14 minutes per account per day (+13% wall clock)** and costs three blocker-grade defects. The pointer advance records `lastPostId: res.dealtIds[0]` — the **first** post of the batch — and `postsToday: _pt + 1`, both hardcoded to one-post-per-cycle. Returning N posts advances the pointer by ONE while delivering N, so the next cycle re-posts the rest: measured **519 deliveries against a 30/day quota (17.3×) and 479 duplicate (post,group) pairs in a single round** — closed form ∑(30…1) = 465 posts/agent/day, 93.5% of them re-posts, into the same groups, on one IP whose anti-spam floors are bypassed. Campaign-plan has no durable per-(post,group) guard (the partition guards against *other* agents; the pointer is the only guard against an agent re-delivering its *own* slice). The crash-fold (`Math.min(N, _priorToday + 1)`) and the single-slot `_owed` ledger are hardcoded the same way. It would also silently delete a guard: the per-IP launch throttle fires once per account **per cycle**, so at 30 cycles every post's first publish is spaced 5–13s, and at 1 cycle 29 of 30 get none. The safe route to the same volume is the dial.
- **[MEDIUM][re-burst route] `pendingPlanBatchId` was being wiped from disk on the first post of every cycle — a bug introduced in 1.0.132 and caught before release.** `store.saveRotation` is a **wholesale replace**, and the rotation-state literal was copy-pasted at **six** call sites; only one carried the new field, so the other five deleted it — including the per-delivery save. The frozen-plan disclosure shipped in 1.0.132 was therefore **dead on arrival**, re-opening the exact Start-Fresh re-burst route it was written to close. Same field-by-field hazard as the `runSeq` / `idleReason` result rebuild, one layer down. All six sites now route through a single builder (`_saveRotationWith`) and pass only what genuinely differs; Start Fresh still clears the marker explicitly. Pinned by four tests that round-trip through **disk** — every prior test asserted the in-memory value, which is why nothing caught it, plus a structural test that fails if any future field goes missing.

## [1.0.132] — 2026-07-17 — ADR-0023 Phase 3: the dashboard tells the truth, and a benched agent can no longer wedge the campaign forever

Suite **447/447** (was 434). Display + attribution work, plus one **total-stop** engine bug. No delivery guard is touched and no dial changes default.

**v1.0.131 overstated its own fix — the correction matters more than the entry did.** It claimed *"a benched account was reported as having FINISHED its work"* was closed. Phase 2 fixed the two **log** surfaces and left the one the operator actually watches: the **Live Operations row still said DONE.** The orchestrator's own comment predicted it verbatim — *"idles all run, reading as a healthy 'done' on the dashboard with no stated reason."*

- **[MEDIUM][total-stop] A BENCHED agent could wedge the campaign PERMANENTLY.** The campaign-plan picker's empty-slice guard sat **above** the owed block, so an agent the spread pass benched could never reach its own discharge path. A benched agent is still a key of `agentLists` (Pass 2 assigns `[]`, it does not delete), so `_campaignAllFinished`'s owed check saw the entry and returned **false forever**: the reloop never fires, the campaign stops dead, and it survives restarts (plan and ledger are both persisted; reconcile re-freezes). The ledger sweep will not reap it either — it only skips entries whose *mode* cannot discharge, and campaign-plan can. The operator sees the stall-breaker's generic *"check your accounts"* — a total misdiagnosis whose end point is Start Fresh. Proven by execution (pick `[]`, owed retained, `allFinished` false). **Now:** only an agent that actually owes takes the new path, so the common benched case is byte-identical — it does not even evaluate the quota gate, which would spend its manual-Start one-shot. Discharging owed groups is not a double-post: they are the **un-reached** groups of a post the agent already partially delivered, and the run is scoped to exactly those. A **stale** obligation (post deleted / groups un-assigned) is now dropped on a claim instead of wedging too. Dormant while every cluster is K=1; it arms the moment accounts are re-enabled into shared group-sets. +3 tests, mutation-verified.
- **[MEDIUM][truthfulness] An account that was never given work rendered as ✅ DONE.** The no-posts return carries no flag and no errors, so the pool's `let _finalState = 'done'` default caught it and wrote `state:'done', action:'done'`. Measured on a real multi-cycle run: a benched agent and a quota-paced agent were **indistinguishable from one that delivered its whole slice**. The reason existed — it was computed, logged, and thrown away. Now the picker returns it and the row badges **IDLE** with the cause (benched / parked / reserve-held / quota / spacing floor / misconfig). `slice-done` deliberately keeps **DONE** — that agent really did finish. The row switches on a **code**, never the prose, so a reworded string cannot silently move a badge.
- **The fix had to survive the pool's field-by-field result rebuild.** That rebuild is an explicit allow-list, and this codebase has been bitten by it once already (`runSeq` omitted → always `undefined` → the R5 clean-commit never recorded). Omitting `idleReason` there would have shipped a **silent no-op that still passed any test asserting on `_runAccount`'s own return** — so the new tests assert on the far side of the rebuild, which is the only place the omission is visible. Mutation-verified: dropping the field fails exactly the 2 new tests while all 4 pre-existing N-cycle tests stay green — the precise false-confidence the harness exists to prevent.
- **[MEDIUM][truthfulness] The plan claimed "~30 day(s)" for a slice that burns in 2.** The plan hands each agent one post per **cycle**, and `cyclesPerDay` of those fire per day — the "1 post/agent/day" model dates from when `cyclesPerDay` was 1. At the live settings (`cyclesPerDay:20`, `cycleGapMin:2` ⇒ a 1-minute spacing floor ⇒ the quota binds at 20/day) a 30-post slice spans **2 days, not 30 — a 15× overstatement** of how long a group is shielded from the whole library, on a fleet that re-loops. Measured with the real engine, not arithmetic. The logs now read `30 cycle(s) ≈ 2 day(s) at up to 20/agent/day`. **The Plan panel was already correct** (it says *"-cycle plan"*) — the engine's log was the liar, so the log was aligned to the panel and not the reverse. At `cyclesPerDay:1` cycles and days coincide and every label keeps its old meaning.
- **[MEDIUM] The seats readout manufactured the inference it exists to prevent.** Three defects, all operator-facing: (1) the dedupe key held the parts but **not the dial**, so editing `campaignMinAgents` to an inert value — the *common* case, since the floor does nothing below the natural `Keff` — recomputed an identical plan, matched the key, and printed **nothing**; the one readout that shows the dial's effect was silent exactly when an answer was needed. (2) It echoed `(Minimum active agents = 6)` **without checking the floor bound anything**. It now says `seating N extra account(s)` or `not binding`. (3) It printed the raw cluster signature (`[group-1784033239927|group-…::]`) instead of group names. It also no longer prints a **seats ratio when nothing is benched** — `1/1 seat(s)` reads as a cap, and the response to a cap is to raise the dial, get an identical plan, and reach for Start Fresh. Silence when there is no bench; when the dial is *set but inert*, it explains the inertness rather than going quiet.
- **[MEDIUM][re-burst route] The frozen plan looked like an ignored edit.** The Plan panel renders the **persisted** plan, frozen for the current round by design (ADR-0019) — so straight after an edit it shows the old shape, indistinguishable from *"my change was ignored"*. The only button that looks like it applies the change is **Start Fresh**, which clears the plan, pointers, `_owed` and the dealt-set and re-delivers the **whole library from #1 to every group**, unpaced, on one IP whose anti-spam floors are bypassed. The engine already knew (`_pendingPlanBatchId`) but kept it **in memory only** — and Start Fresh is reachable only while **stopped**, exactly when that marker was always null. It is now persisted, reloaded defensively (an older rotation file has no key, so an upgrade cannot show a phantom edit), and surfaced as a calm blue *"edit pending — applies automatically at the next round"*. Not an alarm: the correct action is **none**.
- **[LOW] Live Operations contradicted itself on the two states that most need a human.** `checkpoint` and `not_logged_in` are emitted but had no `LIVE_OPS_META` entry, so they fell through to the default and rendered a calm grey **QUEUED** while the summary counted them under *"need attention"*. An unnoticed checkpoint backs off exponentially to a 24 h cap and quietly kills the account. Added. (`needs_verification` is the inverse — the pool collapses that flag into the `needs_login` state before the drop-flag branch, so the entry is dead; kept as defensive coverage and labelled as such.) `idle` is excluded from the "need attention" count — a bench is the planner working as designed.
- **The N-cycle harness can now see the dashboard.** It records every per-account state transition at `_setAcctState` — not off the `automation-progress` emit, which coalesces on a ~400 ms throttle and would drop the exact transition under test. Instance-level shadow, so it cannot leak into another test in the same process.

## [1.0.131] — 2026-07-17 — ADR-0023 Phases 1+2: seat the benched accounts, and stop lying about why they idle

Suite **434/434** (was 412). Implements ADR-0023 Phases 1–2. **Nothing changes on a live fleet until the operator opts in** — the new dial defaults to `0` (off) and the floored expression is algebraically identical to the old one at `0`.

**Read this before turning the dial on.** Implementing Phase 1 refuted two things the ADR implied, both now recorded in it:

- **The floor buys ZERO extra posts.** A group-set delivers its post-set exactly once per round for *any* `Keff`. Flooring redistributes the same work across more accounts, so the set finishes in **fewer days** and then idles until the pace-setter catches up. Measured at K=6/P=30: slices go `[8,8,7,7,0,0]` → `[5,5,5,5,5,5]` — the set spans **5 days instead of 8, ≈ +58% daily volume** into its groups, on an IP whose anti-spam floors are already bypassed (`speedMode:'max'`, `dailyCap:0`). The prize is warm, rotating accounts — not throughput. **Turning it on is an operator decision that wants a live measurement first.**
- **The dial is inert below the natural `Keff`.** `Keff` is already `ceil(posts / globalMaxLen)` and the floor only raises it — on the live topology the natural value is **4**, so `campaignMinAgents` of 2/3/4 does *nothing*; un-benching all five accounts needs **6**. A dial whose effect you cannot see is a dial that gets misused, which is why the seats readout below is not optional.

- **`campaignMinAgents` (new, default 0 = off) floors the campaign spread pass.** Pass 2 paces a fast group-set by benching its surplus agents so every set spans the slowest set's day-count — that is why healthy accounts sat out entire rounds. The floor sets a minimum. `min(K, …)` stays the **outer** clamp so it can never invent a seat, and the pace-setting set is untouched. The partition property is preserved for every floor value (221,400-combo sweep: coverage exact, 0 dupes, `Keff <= K` always). **It lands with no fingerprint change and no Start Fresh** — `batchId` is provably unmoved (identical at dial 0/4/99/absent/garbage), and the round boundary recomputes the plan unconditionally, so an edit applies at the next round on its own. Reloop timing is unchanged: `globalMaxLen` is fixed by Pass 1 *before* Pass 2 runs, so flooring can only ever shorten a slice.
- **[MEDIUM][no-delivery] The clamp is load-bearing, not hygiene.** `clampSettings` has no key whitelist, so an unknown key persists **untouched** — a hand-edited `data.json` or the HTTP API could land `campaignMinAgents:'garbage'` in Pass 2, making `Keff` **NaN**. `rank < NaN` is false for *every* rank → **every agent benched, the campaign delivers nothing at all.** Clamped in the store *and* re-clamped defensively in the engine; pinned by a test.
- **[MEDIUM][truthfulness] A benched account was reported as having FINISHED its work.** A benched agent has an empty slice, so `_campaignNextIdx` returns `{idx:0, len:0}` and `0 >= 0` read as done — the planning header printed **"✓ slice complete"** for an account that was never given anything, while the run log simultaneously called it *"already dealt / posted today"*. Two wrong answers per agent per cycle, disagreeing with each other. This is why a five-account bench was misread and mis-blamed on the planner. A new read-only `_campaignIdleReason` now names the actual cause — benched / parked / reserve-held / quota / spacing floor / slice finished / post shortage — at both existing surfaces. It mirrors the picker's gate order *and* its inputs, mutates nothing, and never spends the manual-Start one-shot.
- **The 🪑 Campaign Plan seats readout.** States seats-per-cluster and echoes the dial on every plan build (log-on-change). Without it the natural `Keff` is invisible, the dial reads as broken, and the operator's next move is **Start Fresh** — a whole-library re-burst. That inference, not any code path, is the most plausible re-burst route in this phase.
- **Neither the readout nor the label ever recommends a remedy that cannot work.** A set can never seat more agents than it has posts, so surplus agents on a short library are benched by *arithmetic* and no floor will move them. Both surfaces now split "the spread pass benched you" (raise the dial) from "your set has more accounts than posts" (add posts / move accounts) — otherwise at the dial's own maximum the advice read *"the floor is 100 — raise it"*.
- **[LOW][truthfulness] The spacing floor was reported as a spent daily quota.** `_dailyQuotaBlocks` answers "blocked" for two unrelated reasons, and the operator's fix differs for each. At `cyclesPerDay=20` with **one** post delivered, the label claimed today's quota was used up. Split into the real quota (resumes tomorrow) vs the anti-burst spacing floor (resumes on its own, nothing to fix).
- **N-cycles-in-ONE-process test harness (`tests/helpers/ncycle.js`, +4 tests).** The head of `_loop` re-reads disk every cycle and `start()` resets the per-run flags, so a test that models the next cycle as a new process watches the bug get reconstructed away before it asserts — which is how a green suite has repeatedly coexisted with live recurring double-posts. The harness holds **one `_loop` invocation** open across several cycles and real round boundaries, with the real picker/pointer/owed/planner underneath and only the worker and the clock faked. It keys its no-duplicate ledger by **round** (a loopCampaign reloop re-delivers the whole library by design) and **fails loudly if it is starved** — a truncated run would make every "no duplicates" assertion pass for the worst possible reason.
- **[LOW] Stale comment corrected.** A `_loop` comment still asserted, in the present tense, that a moved pointer *"yields -1 → idx 0 → the agent RESTARTS its slice"*. That restart was fixed in v1.0.116 (`_campaignNextIdx` treats an unresolvable pointer as CONSUMED); the surviving failure mode is **inverted** — silent under-delivery, not a re-burst. A maintainer trusting it would hunt a phantom double-post.

## [1.0.117] — 2026-07-16 — Closes the last three from the v1.0.115 sweep

Suite **383/383**, antispam 34/34. The v1.0.115 sweep's findings are now **11 of 11 closed.**

- **[MEDIUM][double-post] daily-rotation could wrap WITHIN one local day.** The pointer advance is `% list.length`, so when `cyclesPerDay` exceeds the agent's eligible library the pointer wrapped and re-delivered a post the agent had **already posted today** — same groups, ~45–90 min apart, on the one shared IP. daily-rotation legitimately re-delivers across **days** (which is why it has no durable per-(post,group) guard), but a same-day repeat is a spam burst, not a rotation — and the `cyclesPerDay` gate only counts N; it never noticed the library was smaller than N. Now capped at **one full library pass per day** (`postsToday` is persisted and crash-fold-reconstructed, so a restart can't reset it into another pass). Only ever reduces posting; `N <= library size` is byte-identical. Placed **after** the owed override, so discharging owed groups still works — that finishes un-reached groups of an existing post and is not a same-day repeat. +1 test, mutation-verified.
- **[MEDIUM][false-complete] `_emitCompletionReport` contradicted itself.** It logged the parked-slice warning and then still fired "🎉 every post published" in the same call, and the parked count was the **whole slice** rather than the undelivered remainder (overstating the loss and disagreeing with the posted counts). The remainder is now computed by mirroring `_campaignNextIdx` exactly — including its "unresolvable pointer ⇒ consumed" rule, so the two can never disagree — with deleted posts dropped, and it now **blocks** the completion claim alongside `undeliv`/`noLink`.
- **[LOW] The 📝 edit notice advertised an escape hatch that does not exist.** "Stop → edit → Start" does nothing: the plan is persisted with the rotation state and reloaded on Start, so a restart re-freezes the same plan. Nor can it be made to work safely — clearing only the plan on Start repartitions against surviving pointers, and an agent whose `lastPostId` lands in another slice is then treated as consumed and silently strands its remainder; resetting pointers too re-delivers already-live posts (the re-burst the freeze exists to prevent). The notice now names the **real** hatch: the edit applies automatically at the next round boundary, or immediately via **Start Fresh** — which deliberately clears the plan, pointers, `_owed` and the dealt-set and re-delivers the whole library from #1.
