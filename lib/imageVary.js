...
async function varyImage(srcPath, seed) {
  ...
  try {
    ...
    // Tiny tone shifts the eye won't notice but the hash will.
    img.brightness((rng() - 0.5) * 0.06);                                   // ±3%
    img.color([{ apply: 'hue', params: [Math.round((rng() - 0.5) * 8)] }]); // ±4°
    ...
    const out = path.join(os.tmpdir(), `zpv-${Date.now()}-${Math.floor(rng() * 1e6)}.${ext}`);
