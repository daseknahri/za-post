...
async function varyImage(srcPath, seed) {
  ...
  try {
    ...
    // Surface WHY (was a bare `catch { return null }` → every failure got blamed on "webp", which misdiagnosed the real
    // cause on this fleet: all library images are JPEG, and the failures were tmpdir writeAsync errors on a near-full disk
    // under heavy browser load. Record the reason so the caller can log the TRUTH instead of a phantom format problem.
    _lastError = (e && e.message) || String(e);
    return null;
  }
}
let _lastError = '';
// The reason the most recent varyImage() returned null (empty if it succeeded). Lets the worker log the real cause
// (e.g. "ENOSPC: no space left" / a genuine unreadable format) instead of always blaming webp.
function lastVaryError() { return _lastError; }
// Cheap magic-byte sniff: is this file ACTUALLY webp (RIFF....WEBP)? So the log only says "webp" when it's true.
function isWebp(srcPath) {
  let fd = null;
  try { fd = fs.openSync(srcPath, 'r'); const b = Buffer.alloc(12); fs.readSync(fd, b, 0, 12, 0); return b.slice(0, 4).toString('ascii') === 'RIFF' && b.slice(8, 12).toString('ascii') === 'WEBP'; }
  catch { return false; }
  finally { if (fd !== null) try { fs.closeSync(fd); } catch {} }
}

module.exports = { varyImage, available, lastVaryError, isWebp };
