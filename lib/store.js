...
const DEFAULT_SETTINGS = {
  ...
  tabsPerBrowser: 1,          // MULTI-TAB posting: 1 = classic one-group-at-a-time (default, byte-identical). 2..4 = per
...
function clampSettings(s) {
  ...
  for (const _b of ['commentWithImage', 'autoDeletePosted', 'hideBrowser', 'enableTunnel', 'resumeOnStartup', 'launchOnStartup', 'loopCampaign', 'varyContent', 'varyImages', 'randomizeLinks', 'staggerAccounts', 'postThenComment']) { if (_b in out) out[_b] = !!out[_b]; }
...
function loadProgress() {
  ...
  try { _progressCache = JSON.parse(stripBom(fs.readFileSync(progressFile(), 'utf8'))) || { days: {} }; }
  catch { try { _progressCache = JSON.parse(stripBom(fs.readFileSync(progressFile() + '.bak', 'utf8'))) || { days: {} }; } catch { _progressCache = { days: {} }; } }
  if (!_progressCache.days) _progressCache.days = {};
  return _progressCache;
}
function saveProgress(obj) {
  try { _progressCache = obj || { days: {} }; try { if (fs.existsSync(progressFile())) fs.copyFileSync(progressFile(), progressFile() + '.bak'); } catch {} writeFileAtomic(progressFile(), JSON.stringify(_progressCache, null, 2)); return true; } catch { return false; }
}
...
function recordProgress(record) {
  try {
    ...
    const led = loadProgress(); led.days = led.days || {};
