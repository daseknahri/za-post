...
const DEFAULT_SETTINGS = {
  ...
  waitInterval: 120,    // minutes between full posting cycles
  accountDelay: 2,      // minutes between parallel batches
  postsPerGroup: 1,     // posts each account makes per group per cycle (0 = no limit; unique modes always 1)
  groupDelay: 180,      // seconds between groups within one account (3 min; below ~120s is high-risk)
...
function normalizeAccount(a) {
  ...
  const out = { ...a };
  if (out.daily && typeof out.daily === 'object') out.daily = { date: out.daily.date || '', count: Number(out.daily.count) || 0 };
  if (out.rlStrikes !== undefined) out.rlStrikes = Number(out.rlStrikes) || 0;
  if (out.rateLimitedUntil !== undefined) out.rateLimitedUntil = Number(out.rateLimitedUntil) || 0;
  return out;
}
...
function clampSettings(s) {
  ...
  // Keep the comment-delay window coherent: if BOTH bounds are present and reversed, swap them.
  if ('commentDelayMin' in out && 'commentDelayMax' in out && out.commentDelayMin > out.commentDelayMax) {
    const t = out.commentDelayMin; out.commentDelayMin = out.commentDelayMax; out.commentDelayMax = t;
  }
  return out;
}
...
module.exports = {
  ...
  saveBase64Image, loadRotation, saveRotation, sanitizeProfile, todayKey, dailyRolledOver, dailyUsed,
