...
const DEFAULT_SETTINGS = {
  ...
  defaultPace: 'normal', // per-account pace fallback ('safe' 2× / 'normal' 1× / 'fast' 0.5× / 'turbo' 0.25× / 'instant' 0.1×) when an account has no own pace (see worker.applyPace)
  ...
  capturePostLinkFromNetwork: false, // (two-phase only) capture OUR post's permalink from Facebook's OWN publish
                              ...
                              // Opt-in/experimental until validated live; falls back to the feed-scan if nothing matched.
  // ---- completion engine --------------------------------------------------------------
...
function normalizeAccount(a) {
  ...
  // PACE profile (per-account timing override): 'safe'/'normal'/'fast'/'turbo'/'instant' are valid; anything else
  // drops to undefined so the account inherits settings.defaultPace (the right "unset" sentinel — distinct from a
  // stored 'normal'). 'instant' MUST be here — it's a real per-account tier (worker.applyPace) and was being wiped on load.
  if ('pace' in out) out.pace = ['safe', 'normal', 'fast', 'turbo', 'instant'].includes(out.pace) ? out.pace : undefined;
...
function normalize(raw) {
  return {
    ...
    settings: { ...DEFAULT_SETTINGS, ...migrateTimingRanges(raw.settings), postSets: Array.isArray(raw.settings && raw.settings.postSets) ? raw.settings.postSets.filter((s) => s && s.id && s.name).map((s) => ({ id: String(s.id), name: String(s.name) })) : [] },
...
function clampSettings(s) {
  ...
  if ('speedMode' in out) out.speedMode = ['instant', 'turbo', 'fast', 'normal', 'slow'].includes(out.speedMode) ? out.speedMode : 'normal';
  ...
  if ('defaultPace' in out) out.defaultPace = ['safe', 'normal', 'fast', 'turbo', 'instant'].includes(out.defaultPace) ? out.defaultPace : 'normal'; // GLOBAL fleet fallback — all 5 tiers valid (matches the per-account `pace` set + the Settings dropdown). A fleet-wide turbo/instant is the owner's call for an all-trusted fleet; the worker's anti-spam floors still apply regardless of pace.
  ...
  if ('cyclesPerDay' in out) out.cyclesPerDay = Math.max(1, Math.min(8, Math.round(Number(out.cyclesPerDay) || 1))); // daily mode: cycles/day (1..8); the worker+orchestrator read the same clamp
  ...
  if ('cycleGapMin' in out) out.cycleGapMin = (Number(out.cycleGapMin) > 0) ? Math.max(5, Math.min(720, Math.round(Number(out.cycleGapMin)))) : 0; // 0 = use waitInterval; else 5..720 min
  ...
  for (const _b of ['commentWithImage', 'autoDeletePosted', 'hideBrowser', 'enableTunnel', 'resumeOnStartup', 'launchOnStartup', 'loopCampaign', 'varyContent', 'varyImages', 'randomizeLinks', 'staggerAccounts', 'postThenComment', 'capturePostLinkFromNetwork']) { if (_b in out) out[_b] = !!out[_b]; }
...
function loadProgress() {
  ...
  if (raw != null) {
    try { _progressCache = JSON.parse(stripBom(raw)) || { days: {} }; }
      ...
      try { fs.renameSync(progressFile(), progressFile() + '.corrupt-' + Date.now()); } catch {}
    }
  }
...
function recordProgress(record) {
  try {
    ...
    // Mutate a CLONE, not the shared _progressCache — saveProgress commits it to the cache only on a successful write,
    // so a failed write (ENOSPC / locked .tmp) leaves the in-memory ledger == disk instead of diverging for the process.
    const led = JSON.parse(JSON.stringify(loadProgress())); led.days = led.days || {};
    ...
    bump(status, +1);
    day.items[key] = { account: record.account, postId: record.postId, caption: record.caption || '', groupId: record.groupId, group: record.group || '', status, comment: record.comment || '', ts: record.ts || when.toISOString() };
...
function clearProgress() { try { return saveProgress({ days: {} }); } catch { return false; } }
module.exports = {
  init, load, save, update, updateModeration, updateComments, normalize, blank, DEFAULT_SETTINGS, consumeLoadIssue,
  accountDir, profileDir, cookiesFile, sanitizeName, readCookies, writeCookies, normalizeCookie, writeFileAtomic,
  ...
  appendReport, reportFile, reportCsvFile, loadProgress, recordProgress, clearProgress,
