...
const DEFAULT_SETTINGS = {
  ...
  resumeOnStartup: true,   // #2: auto-resume an interrupted run after shutdown/crash (default ON for the always-running machine — gated on run-active-from-crash AND work-remaining, so it never resurrects a finished run; toggle off to disable)
  ...
  campaignMinAgents: 0,    // 0 = OFF (today's behavior). CAMPAIGN PLAN only: the spread pass (Pass 2) benches agents in a
                           // FAST group-set so it spans the SLOWEST set's day-count; this floors how many agents that set
                           // keeps active. Capped by the set's own agent count, so it can NEVER invent a seat, and it never
                           // touches the pace-setting (slowest) set. It adds NO throughput — the set still delivers the same
                           // posts each round, just across more accounts in FEWER days, i.e. a HIGHER daily volume into that
                           // set's groups. Watch the "🪑 Campaign Plan seats" log line to see what it actually changed.
  ...
  cyclesPerDay: 1,            // DAILY mode: how many cycles fire per LOCAL day (1 = the classic one-cycle/day; 2..N = that
                              // many cycles, one after the other, then rest until tomorrow's fire time). Each account still
                              // posts at most cyclesPerDay times/day AND never re-posts the same post to the same group
                              // (pointer advance + dealt-set are unchanged) — N only relaxes the daily PACING.
                              // READ THIS BEFORE RENAMING IT: campaign-plan and daily-rotation deliver exactly ONE post per
                              // account per cycle, and _dailyQuotaBlocks caps the day at N — so for those modes this dial IS
                              // "posts per account per day". The two numbers are the same by construction, not by accident.
                              // That is why the ceiling is a real product limit and not hygiene: at the old ceiling of 20 an
                              // operator who wanted 30 posts/day simply could not express it. cyclesPerDay is still the
                              // per-account DAILY quota; postsPerCycle (below) decides how many of them land per cycle.
  postsPerCycle: 1,           // CAMPAIGN-PLAN: how many of its slice an agent delivers in ONE cycle (1 = the classic
                              // one-post-per-cycle; 2..N = back-to-back, no barrier between them).
                              // WHY IT EXISTS: a cycle is a BARRIER — every account must finish before the next starts, so
                              // an agent that ends early IDLES until the slowest one lands. Over N cycles you pay the
                              // SLOWEST agent's time N times (N × E[max]) instead of once (max of the per-agent sums).
                              // Measured on the live logs: agents sat idle ~75% of each cycle. Raising this collapses the
                              // day's N barriers into one continuous run per agent.
                              // NEVER EXCEEDS THE DAILY QUOTA: the picker asks for min(postsPerCycle, cyclesPerDay - postsToday),
                              // so this changes WHEN the day's posts land, never HOW MANY. 1 = byte-identical to the old engine.
                              // THE INVARIANT THAT MAKES IT SAFE: the posts loop STOPS at the first post that is not FULLY
                              // delivered, so a cycle has at most ONE partial and it is always the LAST — which is exactly
                              // what the single-slot _owed ledger can represent. Without that stop, N partials in one cycle
                              // would need an owed data-model change, and the un-representable ones would strand.
  ...
  // ---- full-batch-daily: a UI PRESET, not an engine mode ------------------------------
  fullBatchDaily: false,      // when ON, the UI hides the cyclesPerDay + postsPerCycle dials and derives them from the
                              // batch: each campaign account posts its WHOLE share of its batch in ONE daily run, then
                              // stops ("deliver all, then done"). The ENGINE never reads this flag — it reads the
                              // cyclesPerDay/postsPerCycle/loopCampaign/completionMode this preset writes at save time
                              // (all values the multi-pull path already runs safely). Persisted only so the UI re-hides
                              // the dials on reload. Volume = the number of posts you put in the batch (see campaignMaxSlice).
...
function normalize(raw) {
  ...
  return {
    ...
    // #16: clamp/coerce numerics on the LOAD path too (not just IPC-save) — a hand-edited data.json with a string/NaN/
    // negative numeric (e.g. "dailyCap":"5") otherwise reaches the engine as-typed, where Number.isFinite("5") is false →
    // the ban-safety cap silently switches OFF. clampSettings shares the same clamp for load + save; defaults are all
    // in-bounds so this is a no-op for a clean file.
    settings: clampSettings({ ...DEFAULT_SETTINGS, ...rs, postSets: Array.isArray(raw.settings && raw.settings.postSets) ? raw.settings.postSets.filter((s) => s && s.id && s.name).map((s) => ({ id: String(s.id), name: String(s.name) })) : [] }),
...
// The per-account daily POST quota. For campaign-plan / daily-rotation this is what `cyclesPerDay` actually means:
// those modes deliver exactly one post per account per cycle, and _dailyQuotaBlocks stops the account at N — so
// "cycles per day" and "posts per account per day" are the same number by construction.
//
// THE CEILING IS A PRODUCT LIMIT, NOT HYGIENE. It was 20, duplicated as a bare `Math.min(20, …)` in SEVEN places
// (store, the quota gate, the crash-fold, the daily-schedule gate, the round reset, and two label helpers). An
// operator who wants 30 posts/day could not express it, and the obvious-looking alternative — "make an account post
// more per cycle" — is a re-burst: the pointer advance records dealtIds[0] and postsToday+1, both hardcoded to
// one-post-per-cycle, so returning N posts re-delivers N-1 of them on the next cycle. Raising this dial is the safe
// way to get the same volume. Centralised here so the seven copies cannot drift apart again.
const MAX_POSTS_PER_DAY = 50;
function dailyPostQuota(settings) {
  return Math.max(1, Math.min(MAX_POSTS_PER_DAY, Math.round(Number((settings || {}).cyclesPerDay) || 1) || 1));
}

// FULL-BATCH-DAILY derivation. Given the enabled campaign-plan accounts + posts, compute the per-account SLICE the
// planner will hand each account, and return the LARGEST slice across all batches (group-sets). That maxSlice is the
// value the preset writes into BOTH cyclesPerDay (the daily quota) and postsPerCycle (posts delivered per cycle) so
// every account can deliver its whole share in one daily run. Mirrors _computeCampaignPlan's clustering (accounts that
// share a sorted group-set are one batch) and its per-account split (ceil(clusterPosts / accountsInCluster)), and
// respects each batch's post-set (postSetId) so a batch drawing a restricted set is sized by that set, not the whole
// library. Clamped 1..MAX_POSTS_PER_DAY. Returns { maxSlice, batches:[{groups,accounts,posts,slice}], capped }.
// capped=true means some batch needs >MAX_POSTS_PER_DAY/account (can't finish in one day → the UI must warn).
function campaignMaxSlice(accounts, posts) {
  const acc = (accounts || []).filter((a) => a && a.enabled !== false && !a.isModerator && a.standby !== true
    && (a.postingOrder || '') === 'campaign-plan' && (a.assignedGroups || []).length);
  const allPosts = posts || [];
  const clusters = new Map(); // sig -> { accounts:[], setId }
  for (const a of acc) {
  ...
  const batches = [];
  let maxSlice = 0, capped = false;
  for (const [, c] of clusters) {
    const pool = c.setId ? allPosts.filter((p) => (p.postSetId || null) === c.setId) : allPosts;
    const K = Math.max(1, c.accounts.length);
    const rawSlice = Math.ceil(pool.length / K); // the biggest per-account partition in this cluster
    const slice = Math.max(1, Math.min(MAX_POSTS_PER_DAY, rawSlice));
    if (rawSlice > MAX_POSTS_PER_DAY) capped = true;
    if (slice > maxSlice) maxSlice = slice;
    batches.push({ groups: c.groups, accounts: c.accounts, posts: pool.length, slice, rawSlice });
  }
  return { maxSlice: Math.max(1, maxSlice), batches, capped };
}
// How many of its slice a campaign agent may deliver in ONE cycle. Clamped to the same ceiling as the daily quota: a
// value above it can never be reached anyway (the picker asks for min(this, quota - postsToday)), and railing it here
// keeps a hand-edited data.json or the HTTP API from putting a NaN/huge value on the delivery path. 1 = the classic
// one-post-per-cycle engine, byte-identical.
function postsPerCycle(settings) {
  return Math.max(1, Math.min(MAX_POSTS_PER_DAY, Math.round(Number((settings || {}).postsPerCycle) || 1) || 1));
}
...
function clampSettings(s) {
  ...
  // NOT hygiene — load-bearing. clampSettings has no key whitelist, so an unknown key persists UNTOUCHED: a hand-edited
  // or API-supplied campaignMinAgents:'garbage' reaches Pass 2, makes Keff NaN, and `rank < NaN` is false for EVERY rank →
  // every agent benched → the campaign silently delivers NOTHING. The engine re-clamps defensively too (belt and braces).
  if ('campaignMinAgents' in out) out.campaignMinAgents = Math.round(n(out.campaignMinAgents, 0, 0, 100));
  ...
  if ('realIpMaxConcurrent' in out) out.realIpMaxConcurrent = n(out.realIpMaxConcurrent, 3, 1, 20); // max concurrent browsers on one shared IP (1..20; operator-controlled — raised from 8 so parallelAccounts>8 can take effect on a single IP; matches the orchestrator _realIpMax gate). Higher = more ban exposure on that IP; the operator owns the trade.
  ...
  if ('cyclesPerDay' in out) out.cyclesPerDay = dailyPostQuota({ cyclesPerDay: out.cyclesPerDay }); // daily mode: cycles/day == posts/account/day (1..MAX_POSTS_PER_DAY); every reader goes through this same helper
  if ('postsPerCycle' in out) out.postsPerCycle = postsPerCycle({ postsPerCycle: out.postsPerCycle }); // campaign-plan: posts per agent per CYCLE (1 = classic). Load-bearing: an unclamped NaN reaches the picker's slice() and the delivery path.
  ...
  for (const _b of ['commentWithImage', 'autoDeletePosted', 'hideBrowser', 'enableTunnel', 'resumeOnStartup', 'launchOnStartup', 'loopCampaign', 'varyContent', 'varyImages', 'randomizeLinks', 'staggerAccounts', 'postThenComment', 'capturePostLinkFromNetwork', 'skipInlineVerify', 'fastPublish', 'fullBatchDaily']) { if (_b in out) out[_b] = !!out[_b]; }
...
// #8: append + fsync so the line reaches DISK (not just the OS page cache) before the delivery is relied on as the
// no-double-post guard — appendFileSync alone survives a force-kill but NOT an OS crash / power loss in the writeback
// window, exactly the failure this journal's header claims to cover. One fsync per post is negligible vs the ~30s floor.
// A torn final line (kill mid-write, before fsync) stays tolerated by loadInflight.
function _appendFsync(file, data) { const fd = fs.openSync(file, 'a'); try { fs.writeSync(fd, data); fs.fsyncSync(fd); } finally { fs.closeSync(fd); } }
function appendInflight(entry) {
  try { _appendFsync(inflightFile(), JSON.stringify(entry) + '\n'); return true; }
...
function appendObligation(entry) {
  try { _appendFsync(obligationFile(), JSON.stringify(entry) + '\n'); return true; } // #8: fsync (see _appendFsync) so a just-journaled held/comment obligation survives power loss, not only a force-kill
  catch { return false; }
}
...
function saveModeration(obj) {
  try {
    ...
    // #2: refresh .bak ONLY from a PARSEABLE primary — a corrupt (truncated/garbled) primary must NOT clobber the good
    // backup (a copy from a bad primary + a failed atomic write would lose BOTH copies). Mirrors saveProgress's guard.
    try { if (fs.existsSync(moderationFile())) { JSON.parse(stripBom(fs.readFileSync(moderationFile(), 'utf8'))); fs.copyFileSync(moderationFile(), moderationFile() + '.bak'); } } catch {}
...
function saveComments(obj) {
  try {
    ...
    // #2: refresh .bak ONLY from a PARSEABLE primary (mirrors saveModeration / saveProgress) — never clobber a good backup.
    try { if (fs.existsSync(commentsFile())) { JSON.parse(stripBom(fs.readFileSync(commentsFile(), 'utf8'))); fs.copyFileSync(commentsFile(), commentsFile() + '.bak'); } } catch {}
...
module.exports = {
  ...
  clampSettings, preserveAttentionStatus, dailyPostQuota, postsPerCycle, MAX_POSTS_PER_DAY, campaignMaxSlice,
