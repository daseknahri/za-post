...
function buildPlan(input) {
  ...
  if (method === 'campaign-plan') {
    const cyc = campaignCycleDays(posters, posts, rotation, progress, grp, snippet, settings.campaignMinAgents);
    // FROZEN-PLAN DISCLOSURE. The panel below renders the PERSISTED plan, which is deliberately frozen for the current
    // round (ADR-0019) — so right after an edit it shows the OLD shape and looks like the edit was ignored. Nothing
    // said otherwise, and the only button that looks like it applies the change is Start Fresh, which re-delivers the
    // whole library from #1 to every group. Surfacing the engine's own pending marker is what makes "it is already
    // handled, do nothing" visible. Read defensively: a rotation file from any earlier version has neither key.
    const _cp = (rotation && rotation.campaignPlan) || null;
    const _pending = (rotation && typeof rotation.pendingPlanBatchId === 'string' && rotation.pendingPlanBatchId) ? rotation.pendingPlanBatchId : null;
    const frozen = (_cp && _pending && _pending !== _cp.batchId) ? { batchId: _cp.batchId || null, pendingBatchId: _pending } : null;
    return {
      method, ongoing: !settings.completionMode, todayKey,
      cycleDays: cyc.cycleDays, daysWithProgress: Object.keys(progress).length,
      totals: lifetimeTotals(progress), todayIndex: cyc.todayIndex, days: cyc.days, frozen,
...
function computeCampaignAgentLists(active, posts, roundOffset = 0, minAgents = 0) {
  ...
  if (globalMaxLen > 0) {
    for (const [, ag] of clusters) {
      ...
      // ADR-0023 P1: mirror the engine's campaignMinAgents floor (orchestrator ~3749), same clamp ORDER — min(K,…) outer,
      // so the preview can never show a seat the engine won't fill. Only reached BEFORE the first run persists a plan (see
      // the call site below), which is exactly when the operator is tuning the dial and needs the preview to agree.
      const _minA = Math.max(0, Math.min(100, Math.round(Number(minAgents) || 0)));
      const Keff = Math.min(K, Math.max(1, _minA, Math.ceil(posts.length / globalMaxLen)));
...
function campaignCycleDays(posters, posts, rotation, progress, grp, snippet, minAgents = 0) {
  const cp = rotation.campaignPlan;
  ...
  if (isCurrent) { // draw upcoming cycles for the LIVE round from the agentLists + pointer
    ...
    const agentLists = (cp && cp.agentLists) ? cp.agentLists : computeCampaignAgentLists(active, posts, roundOffset, minAgents);
    const per = rotation.perAccountRotation || {};
