...
let _hwidCache = null; // memoize the FIRST successful read so a later TRANSIENT machineId() throw can't flip a machine's id → wrong "bound to a different machine" lockout
function hwid() {
  if (_hwidCache) return _hwidCache;
  try { const v = machineId ? machineId() : require('os').hostname(); if (v) { _hwidCache = String(v); return _hwidCache; } return null; }
  catch { return null; } // machineId threw (AV/registry lock, truncated REG.exe) → SENTINEL null; callers treat null as "temporarily unreadable" (like readCache's __unreadable). Never fall back to os.hostname() — a DIFFERENT string that hard-mismatched the persisted MachineGuid and tore down a running campaign at the ~6h re-check.
}
...
function decideFromResponse(d, key, now = Date.now()) {
  ...
  if (expires && expires < now) return { valid: false, expired: true, message: 'License expired' }; // defense-in-depth: enforce expiry client-side even if the server affirmed (a buggy/compromised server must not grant an expired license)
  if (d.valid) {
...
async function validate(key, serverUrl, userData) {
  ...
  if (id === null) return decideOffline(key, cache, undefined, undefined, null); // hwid temporarily unreadable → don't POST a null hwid to the server (it'd reject a customer); owner is valid offline, a cached customer stays on the grace path instead of a hard machine-mismatch
...
async function checkCached(userData, serverUrl) {
  ...
  const _id = hwid();
  if (_id === null) return { valid: true, unreadable: true, message: 'License device-id temporarily unreadable — will re-check' }; // transient machineId blip → soft-pass a CACHED license (like readCache's __unreadable) so the ~6h re-validator doesn't tear down a running campaign; re-checks once it clears
  if (c.hwid && c.hwid !== _id) return { valid: false, message: 'License is bound to a different machine' }; // hard mismatch only on a PRESENT, DIFFERENT cached hwid; a falsy cached hwid falls through to re-validate (the server still enforces the device-lock for customers)
