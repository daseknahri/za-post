...
class ProxyHealthManager {
  ...
  save(path) {
    ...
    try {
      ...
      try { const buf = Buffer.from(data); let off = 0; while (off < buf.length) off += fs.writeSync(fd, buf, off, buf.length - off); fs.fsyncSync(fd); } finally { fs.closeSync(fd); } // R1: loop the write so a short write can't torn-write proxy-health.json (mirrors store.writeFileAtomic)
